...
async def _fetch_file_context(db, conversation_id: str) -> str:
    ...
    header = (
        "Files attached to THIS conversation. The user's deictic phrases — "
        "'the document', 'this PDF', 'the uploaded file', 'the attached doc', "
        "'the report I sent', 'what I just uploaded' — ALWAYS refer to one of "
        "the files below, NEVER to the persistent /knowledge-base.\n"
        "\n"
        "Routing for content questions about these files:\n"
        "  • .pdf      -> read_pdf({ file_id })\n"
        "  • .csv/.xlsx/.docx/image -> execute_python with _files[\"<name>\"]\n"
        "  • workspace files        -> workspace_read_file\n"
        "Do NOT call search_knowledge_base when the answer is in one of these "
        "files — search_knowledge_base is ONLY for the user's cross-conversation "
        "knowledge base, not for current-chat attachments.\n"
        "\n"
